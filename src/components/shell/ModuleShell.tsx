  ...
  return (
    <div className="flex flex-col h-[calc(100vh-120px)] bg-ms-dark rounded-3xl overflow-hidden border border-gray-800 shadow-2xl animate-in fade-in zoom-in duration-300 w-full">
      <div className="px-4 py-4 md:px-8 md:py-6 bg-ms-card border-b border-gray-800 flex items-center justify-between">
        <div className="flex items-center gap-3 md:gap-4">
          <button
            ...
            className="p-2 hover:bg-gray-800 rounded-full transition-colors text-gray-400 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ms-blue"
          >
            <ArrowLeft className="w-5 h-5" />
          </button>
          <div>
            <h1 className="text-xl md:text-2xl font-black text-white tracking-tight">{titulo}</h1>
            {subtitulo && <p className="text-xs md:text-sm text-gray-400 font-bold">{subtitulo}</p>}
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
        <nav
          aria-label="Módulos"
          className="w-full md:w-64 bg-ms-header border-b md:border-b-0 md:border-r border-ms-border flex flex-row md:flex-col shadow-xl z-10 p-3 md:p-4 gap-2 overflow-x-auto md:overflow-y-auto no-scrollbar"
        >
          {modulos.map((item) => (
            <button
              key={item.id}

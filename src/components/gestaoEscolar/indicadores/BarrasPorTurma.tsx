...
export interface ItemBarra { rotulo: string; partes: ParteBarra[] }
...
export function BarrasPorTurma({ itens, vazio = 'Sem dados.', rotuloTotal = 'Total' }: BarrasPorTurmaProps) {
  ...
  const linha = (item: ItemBarra, destaque: boolean) => {
    ...
    return (
      <div key={item.rotulo} className={`flex items-center gap-2 text-xs ${destaque ? 'pb-1 mb-1 border-b border-gray-700' : ''}`}>
        <span className={`w-28 flex-shrink-0 truncate ${destaque ? 'font-black text-ms-main uppercase' : 'text-gray-400'}`} title={item.rotulo}>{item.rotulo}</span>
  ...
  return (
    <div className="space-y-1" role="img" aria-label={`${rotuloTotal}: ${total}. ${itens.map((i) => `${i.rotulo}: ${somaItem(i)}`).join('; ')}`}>

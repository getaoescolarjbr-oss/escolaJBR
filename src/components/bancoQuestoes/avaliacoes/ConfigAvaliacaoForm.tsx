...
export function ConfigAvaliacaoForm({ questoes, onVoltar, onContinuar }: Props) {
  ...
  const [disciplina, setDisciplina] = useState(() => Array.from(new Set(questoes.map((q) => q.discipline))).join(', '));
  ...
  const podeContinuar = titulo.trim().length > 0 && turmaIds.size > 0 && (!precisaOnline || prazoEntrega);

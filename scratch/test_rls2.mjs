...
async function run() {
  ...
  // Try insert with professor_id = user.id
  console.log('\n--- Testando insert atividades_diárias com professor_id = user_id ---');
  let res = await supabase.from('atividades_di\u00e1rias').insert({
      ...
      bimestre_id: 2,
      data: hoje,
      descricao: 'Teste professor_id user'
  }).select('id');
  console.log(res.error ? res.error.message : 'OK: ' + res.data[0].id);

  // Try insert with id_do_professor = user.id
  console.log('\n--- Testando insert atividades_diárias com id_do_professor = user_id ---');
  res = await supabase.from('atividades_di\u00e1rias').insert({
      id_do_professor: authData.user.id,
      turma_id: turmaId,
      disciplina_id: disciplinaId,
      bimestre_id: 2,
      data: hoje,
      descricao: 'Teste id_do_professor user'
  }).select('id');
  console.log(res.error ? res.error.message : 'OK: ' + res.data[0].id);

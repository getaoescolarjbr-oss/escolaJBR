...
async function run() {
  console.log('Logging in as claysson.127082@edutec.sed.ms.gov.br...');
  ...
  console.log('\n--- Querying Professores ---');
  const { data: prof, error: profErr } = await supabase.from('professores').select('*').eq('user_id', authData.user.id).single();
  console.log('Professor Record:', prof, 'Error:', profErr?.message);
